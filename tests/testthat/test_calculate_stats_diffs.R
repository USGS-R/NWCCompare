...
test_that("Original demo for for diff stats works", {
  ...
  enddate <- "2002-09-30" # old end date was 2010-09-30 but NWIS not available after 2004.
  ...
  flow_data_b <- build_nwc_flow_dataset(hucs, 
                                        start_date = startdate, 
                                        end_date = enddate)
  expect_equal_to_reference(calculate_stats_diffs(sites = sites, 
                                                  flow_data_a = flow_data_a, 
                                                  flow_data_b = flow_data_b, 
                                                  yearType = "water", 
                                                  digits = 2),
                            "data/test_calculate_stats_diffs_huc_nwis_statsout.rds")
})