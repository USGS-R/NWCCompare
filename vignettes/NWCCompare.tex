...
{\ttfamily\noindent\itshape\color{messagecolor}{\\Attaching package: 'NWCCompare'

The following objects are masked from 'package:EflowStats':

\ \ \ \ obs\_data, qfiletempf}}\end{kframe}
\end{knitrout}
...
\hlstd{model_url}\hlkwb{=}\hlstr{"http://cida.usgs.gov/nwc/thredds/sos/watersmart/stats/stats-SE-DENSE2-2.03.nc?request=GetObservation&service=SOS&version=1.0.0&offering"}
\hlstd{diffInputsv} \hlkwb{<-} \hlkwd{diffInputs}\hlstd{(model_url)}
\end{alltt}
\begin{verbatim}
Retrieving list of sites in scenario from: 
 http://cida.usgs.gov/nwc/thredds/sos/watersmart/stats/stats-SE-DENSE2-2.03.nc?request=GetCapabilities&service=SOS&version=1.0.0 
Retrieving start and end dates in scenario from: 
 http://cida.usgs.gov/nwc/thredds/sos/watersmart/stats/stats-SE-DENSE2-2.03.nc?request=GetCapabilities&service=SOS&version=1.0.0 
\end{verbatim}
\begin{alltt}
\hlstd{startdate} \hlkwb{<-} \hlstd{diffInputsv[[}\hlnum{1}\hlstd{]]}
\hlstd{enddate} \hlkwb{<-} \hlstd{diffInputsv[[}\hlnum{2}\hlstd{]]}
\hlstd{x_urls} \hlkwb{<-} \hlstd{diffInputsv[[}\hlnum{3}\hlstd{]]}
\hlstd{d_urls} \hlkwb{<-} \hlstd{diffInputsv[[}\hlnum{4}\hlstd{]]}
\hlstd{m_urls} \hlkwb{<-} \hlstd{diffInputsv[[}\hlnum{5}\hlstd{]]}
\hlstd{sites} \hlkwb{<-} \hlstd{diffInputsv[[}\hlnum{6}\hlstd{]]}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hlstd{statsout} \hlkwb{<-} \hlkwd{calculateStatsDiffs}\hlstd{(sites, startdate, enddate, getXMLWML1.1Data, x_urls, getDrainageArea, d_urls, SWE_CSV_IHA, m_urls)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{verbatim}
    site_no lam1Obs lam1Mod lam1Diff     nse   rmse pearson
V1 02186000   176.5   190.3   0.0782 0.55813  142.3  0.7570
V2 02192000  1495.3  1288.5  -0.1383 0.76862 1103.8  0.8815
V3 02219000   206.6   167.7  -0.1880 0.81824  145.0  0.9092
V4 02317500   954.7  1108.7   0.1614 0.01512 1932.2  0.5913
V5 02329600   361.8   312.3  -0.1369 0.18583  744.5  0.5051
\end{verbatim}
\end{kframe}
\end{knitrout}
...
\hlstd{d_urls}\hlkwb{<-}\hlkwd{paste}\hlstd{(drainage_url, sites,} \hlkwc{sep} \hlstd{=} \hlstr{""}\hlstd{)}
\hlstd{statsout} \hlkwb{<-} \hlkwd{calculateStatsGroups}\hlstd{(stats, sites, startdate, enddate, getXMLWML1.1Data, x_urls, getDrainageArea, d_urls)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\end{alltt}
\begin{verbatim}
function (GagedFlowStats, ModeledFlowStats) 
{
    Output <- matrix(nrow = 6, ncol = ncol(GagedFlowStats))
    for (i in 1:ncol(GagedFlowStats)) {
        Output[1, i] <- nse(GagedFlowStats[, i], ModeledFlowStats[, 
            i])
        Output[2, i] <- nselog(GagedFlowStats[, i], ModeledFlowStats[, 
            i])
        Output[3, i] <- rmse(GagedFlowStats[, i], ModeledFlowStats[, 
            i])
        Output[4, i] <- pbias(ModeledFlowStats[, i], GagedFlowStats[, 
            i])
        Output[5, i] <- cor(GagedFlowStats[, i], ModeledFlowStats[, 
            i], method = "pearson")
        Output[6, i] <- cor(GagedFlowStats[, i], ModeledFlowStats[, 
            i], method = "spearman")
    }
    return(Output)
}
<environment: namespace:NWCCompare>
\end{verbatim}
\end{kframe}
\end{knitrout}
